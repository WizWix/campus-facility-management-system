          ...
          <form onSubmit={handleSubmit}>
            <div className="modal-bd">
              <div className="mb-3">
                ...
                <input type="text" className="form-control" value={`${counselor.name} (${counselor.position})`}
                       readOnly/>
              </div>
                ...
                <div style={{flex: 1}}>
                  ...
                  <div style={{display: 'flex', gap: '4px'}}>
                    <select className="form-select" value={startHour}
                      ...
                      {hourOptions.filter(h => h < 17).map(h => <option key={h}
                                                                        value={h}>{String(h).padStart(2, '0')}시</option>)}
                    </select>
                    <select className="form-select" value={startMin}
                ...
                <div style={{flex: 1}}>
                  ...
                  <div style={{display: 'flex', gap: '4px'}}>
                    <select className="form-select" value={endHour} onChange={e => setEndHour(Number(e.target.value))}>
                      {hourOptions.filter(h => h >= 9).map(h => <option key={h}
                                                                        value={h}>{String(h).padStart(2, '0')}시</option>)}
                    </select>
              ...
              <div className="mb-3">
                ...
                <input type="text" className="form-control" value={topic} onChange={e => setTopic(e.target.value)}
                       placeholder="예: 수강신청 변경 문의"/>
              </div>
              <div className="mb-3">
                <label className="form-label">메모</label>
                <textarea className="form-control" rows="3" value={memo} onChange={e => setMemo(e.target.value)}
            ...
            <div className="modal-ft">
              ...
              <button type="submit" className="btn btn-primary"
                      disabled={submitting}>{submitting ? '신청 중...' : '예약 신청'}</button>
            </div>
          </form>
        </div>
      </div>
