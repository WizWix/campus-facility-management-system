...
//   GET  /api/clubs/me          — 내 동아리 목록 (ClubTab)
      ...
      <div>
        ...
        <div className="mypage-profile-card">
          <div className="mypage-profile-row">
            <span className="mypage-label">이름</span><span>{profile.name}</span>
          </div>
          <div className="mypage-profile-row">
            <span className="mypage-label">학번/교번</span><span>{profile.userNumber}</span>
          </div>
          <div className="mypage-profile-row">
            <span className="mypage-label">이메일</span><span>{profile.email}</span>
          </div>
          <div className="mypage-profile-row">
            <span className="mypage-label">역할</span><span>{ROLE_LABELS[profile.role] || profile.role}</span>
          </div>
          <div className="mypage-profile-row">
            <span className="mypage-label">성별</span><span>{GENDER_LABELS[profile.gender] || '-'}</span>
          </div>
          <div className="mypage-profile-row">
            <span className="mypage-label">가입일</span><span>{profile.createdAt?.substring(0, 10)}</span>
          </div>
        </div>
                    ...
                    <tr key={a.id}>
                      ...
                      <td>
                        {a.status === 'REJECTED' && a.rejectReason ? (
                            <button
                                className={`mypage-badge ${STATUS_CLASSES[a.status] || ''}`}
                            ...
                            <span className={`mypage-badge ${STATUS_CLASSES[a.status] || ''}`}>
                    ...
                    <tr key={r.id}>
                      ...
                      <td>
                        {r.status === 'REJECTED' && r.rejectReason ? (
                            <button
                                className={`mypage-badge ${STATUS_CLASSES[r.status] || ''}`}
                                style={{cursor: 'pointer', border: 'none'}}
                                title="클릭하여 거절 사유 확인"
                                onClick={() => alert(`거절 사유:\n${r.rejectReason}`)}>
...
function SeatReservationTab() {
  ...
  useEffect(() => {
    import('../data/api.js').then(({fetchBuildings}) => {
      fetchBuildings()
          .then(buildings => {
            const lib = buildings.find(b => b.slug === 'library' || b.type === 'LIBRARY');
            if (!lib) throw new Error('도서관 건물 정보를 찾을 수 없습니다.');
            setBuildingId(lib.id);
  ...
  async function handleCancel(id) {
    if (!confirm('정말 취소하시겠습니까?')) return;
    try {
      await cancelSeatReservation(buildingId, id);
...
function StudyReservationTab() {
  ...
  async function handleCancel(id) {
    if (!confirm('정말 취소하시겠습니까?')) return;
    try {
      await cancelStudyRoomReservation(buildingId, id);
      setList(prev => prev.filter(r => r.id !== id));
...
// 동아리 역할 — DB ClubRole enum 값 그대로 매핑
// ROLE_PRESIDENT(회장), ROLE_VICE_PRESIDENT/ROLE_MEMBER(동아리원) 두 가지로 표시
const CLUB_ROLE_LABELS = {
  ROLE_PRESIDENT: '회장',
  ROLE_VICE_PRESIDENT: '동아리원',
  ROLE_MEMBER: '동아리원',
};

// 가입 상태 — DB ClubMemberStatus enum: APPROVED / PENDING / REJECTED
...
const MEMBER_STATUS_CLASSES = {
  APPROVED: 'mypage-badge-approved',
  PENDING: 'mypage-badge-pending',
  REJECTED: 'mypage-badge-rejected',
  LEFT: 'mypage-badge-cancelled',
};

// 내 동아리 탭 — 가입한 동아리 목록 (APPROVED + PENDING)
// DB 실시간 반영: GET /api/clubs/me → ClubMemberService.getMyClubs()
// ResponseMyClub 필드: clubId, clubName, slug, presidentName, clubStatus, myRole, memberStatus, joinedAt
function ClubTab() {
  const [list, setList] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  ...
  return (
      <div>
        <div className="mypage-section-header">
          <h3>내 동아리</h3>
          <p className="mypage-section-sub">내가 가입하거나 신청한 동아리 목록입니다.</p>
        </div>
        {list.length === 0 ? (
            <div className="mypage-empty">가입하거나 신청한 동아리가 없습니다.</div>
        ) : (
            <div className="table-responsive">
              <table className="table mypage-table">
                <thead>
                ...
                {list.map(c => (
                    <tr key={c.clubId}>
                      {/* 동아리명 — 클릭 불가, 일반 텍스트로 표시 */}
                      <td>{c.clubName}</td>
                      {/* 성명: 동아리 회장 이름 (presidentName) */}
                      <td>{c.presidentName || '-'}</td>
                      {/* 역할: 승인된 경우에만 역할 표시, 대기 중엔 '-' */}
                      <td>{c.memberStatus === 'APPROVED' ? (CLUB_ROLE_LABELS[c.myRole] ?? '-') : '-'}</td>
                      {/* 가입 상태: DB ClubMemberStatus 실시간 반영 */}
                      <td>
