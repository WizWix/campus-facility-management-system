...
public class CafeteriaService implements ICafeteriaService {
  ...
  public List<ResponseFoodCourtStore> getFoodCourtStores() {
    ...
    Map<Long, List<CafeteriaStoreMenu>> menusByStoreId = allMenus.stream()
        .collect(Collectors.groupingBy(menu -> menu.getStore().getId()));

    return stores.stream()
        .map(store -> {
          List<ResponseStoreMenuItem> menuItems = menusByStoreId.getOrDefault(store.getId(), List.of()).stream()
              .map(menu -> new ResponseStoreMenuItem(
                  menu.getName(),
                  menu.getPrice(),
                  menu.getDiscountPrice() != null
                      ? new ResponseDiscount(menu.getDiscountPrice(), menu.getDiscountLabel())
                      : null,
                  Boolean.TRUE.equals(menu.getPopular())
              ))
              .toList();
          return new ResponseFoodCourtStore(
              store.getId(), store.getName(), store.getDescription(),
              store.getCategory(), store.getRepresentative(), store.getHours(), menuItems
          );
        })
        .toList();
  }
